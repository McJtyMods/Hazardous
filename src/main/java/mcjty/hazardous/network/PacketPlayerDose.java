...
public record PacketPlayerDose(Map<ResourceLocation, Double> values) implements CustomPacketPayload {

    public static final ResourceLocation ID = new ResourceLocation(Hazardous.MODID, "playerdose");

    public static PacketPlayerDose create(FriendlyByteBuf buf) {
        Map<ResourceLocation, Double> values = new HashMap<>();
        int size = buf.readInt();
        for (int i = 0; i < size; i++) {
            values.put(buf.readResourceLocation(), buf.readDouble());
        }
        return new PacketPlayerDose(values);
    }
